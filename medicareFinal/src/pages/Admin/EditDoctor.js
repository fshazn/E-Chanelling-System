...
const profileTitle={
  ...
  const{firstName,lastName,email,contact,username,password}=doctors;
  const onInputChange = (e) => {
    setDoctors({ ...doctors, [e.target.name]: e.target.value });
  };
  const onSubmitForm = async (e) => {
    e.preventDefault();
    await axios.put("http://localhost:8081/doctors", doctors)
  ...
  return (
    <Grid container justifyContent='center'>
        ...
        <Paper style={editPaper}>
            ...
            <Button onClick={()=> {navigate(`/admin/specialities/${specializationId}/doctors`)}}>Cancel</Button>
            </Stack>
        </Paper>
        </form>
    </Grid>
